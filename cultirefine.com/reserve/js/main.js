...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.MENU_ERROR) {
        console.error('Menu data loading failed:', window.MENU_ERROR);
        showErrorMessage('メニューデータの読み込みに失敗しました。メニュー選択機能に制限が生じる可能性があります。\n' + window.MENU_ERROR);
        
        // メニューが利用できない旨を表示エリアに設定
        const menuCalendarScreen = document.getElementById('menu-calendar-screen');
        if (menuCalendarScreen) {
            const treatmentCategories = menuCalendarScreen.querySelector('#treatment-categories');
            if (treatmentCategories) {
                treatmentCategories.innerHTML = `
                    <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <h3 class="text-sm font-medium text-yellow-800">メニューデータ読み込みエラー</h3>
                                <p class="mt-1 text-xs text-yellow-700">システム管理者にお問い合わせください。</p>
                            </div>
                        </div>
                    </div>
                `;
